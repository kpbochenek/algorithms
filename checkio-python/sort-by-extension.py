def sort_by_ext(files: list[str]) -> list[str]:
    fsorted = sorted(files, key=lambda f: (f if f[0] == '.' and '.' not in f[1:] else f.split('.')[-1], f.split('.')[:-1]))
    if '.config' in fsorted:
        fsorted.insert(0, fsorted.pop(fsorted.index('.config')))
    return fsorted
